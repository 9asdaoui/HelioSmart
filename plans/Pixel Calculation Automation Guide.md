...
## Automation Strategy 5: Machine Learning-Based Scale Detection

Train a model to estimate scale from image content:

```python
# py_service/scale_estimator.py
import torch
...
class ScaleEstimator(nn.Module):
    """
    CNN to estimate image scale from visual features
    
    Trained on satellite images with known scales
    """
    ...
    def __init__(self):
        ...
        self.backbone.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 1)  # Output: meters per pixel
        )
...
class ScalePredictor:
    """
    Predict image scale using trained model
    """
    
    def __init__(self, model_path='scale_model.pth'):
        self.model = ScaleEstimator()
    ...
    def predict_scale(self, image_array):
        """
        Predict scale from image
        ...
        # Predict
        with torch.no_grad():
            scale = self.model(input_tensor).item()
        
        # Clamp to reasonable range for satellite imagery
        return max(0.01, min(scale, 10.0))
...
def prepare_training_data():
    ...
    Need images with:
    - Known scale (m/pixel)
    - Diverse locations and zoom levels
    """
    # This would be populated with your labeled dataset
    training_data = []
...
## Automation Strategy 6: Integration with Backend
...
| ML Estimation | Medium (±15%) | High | Medium | Unknown sources |
...
### Phase 1: EXIF Extraction (Week 1)
...
5. **ML Estimation** - As a fallback for unknown image sources

**Recommended Priority:**
1. Implement EXIF extraction (immediate value for drone users)
2. Add zoom-based scale calculation (improves Google Maps integration)
